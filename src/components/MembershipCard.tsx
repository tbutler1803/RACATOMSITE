...
function MembershipCard({ name, price, description, highlights }: MembershipCardProps) {
  ...
  return (
    <>
      <button
        type="button"
        className="min-h-64 md:min-h-80 cursor-pointer group bg-transparent border-0 p-0 w-full text-center"
      ...
      >
        ...
        >
          <h3 className="text-[var(--color-gold-accent)] font-heading mb-2 md:mb-3 text-lg md:text-2xl tracking-wide uppercase">
            {name}
          </h3>
          ...
          <p className="text-base md:text-lg text-[var(--color-cream)] font-bold leading-relaxed mb-2">
            {price === '$3,590' || price === '$2,440' || price === '$2,020' ? 'Individual Membership' : name === 'Corporate / Community' ? 'Group Membership' : 'Individual Membership'}
          </p>
